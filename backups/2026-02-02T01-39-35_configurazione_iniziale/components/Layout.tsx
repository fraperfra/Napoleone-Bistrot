...
const Layout: React.FC<LayoutProps> = ({ children, activePage, setActivePage, lang, setLang }) => {
  ...
  const Logo = ({ light = false }: { light?: boolean }) => (
    <div className="flex items-center gap-3 cursor-pointer" onClick={() => { setActivePage('home'); setIsMenuOpen(false); }}>
      <div className="w-10 h-10 border-2 border-gold rounded-full flex items-center justify-center relative overflow-hidden group">
        <span className="font-serif text-2xl font-bold text-gold group-hover:scale-110 transition-transform">N</span>
        <div className="absolute inset-0 border border-gold scale-90 rounded-full opacity-50"></div>
      </div>
      <div className="flex flex-col">
        <span className={`font-serif text-xl tracking-widest font-bold leading-none transition-colors duration-500 ${light ? 'text-white' : (isHeroHeader ? 'text-white' : 'text-darkGreen')}`}>
          NAPOLEONE
        </span>
        <span className={`text-[10px] tracking-[0.3em] font-sans uppercase opacity-70 transition-colors duration-500 ${light ? 'text-white' : (isHeroHeader ? 'text-white' : 'text-darkGreen')}`}>
          Bistrot • Verona
        </span>
      </div>
    </div>
