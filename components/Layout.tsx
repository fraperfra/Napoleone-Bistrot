    ...
    <div className="min-h-screen flex flex-col">
      ...
      {isMenuOpen && (
        <div className="fixed inset-0 z-[70] flex items-center justify-center p-4 md:hidden">
          {/* Backdrop Blur/Dim */}
          <div 
            className="absolute inset-0 bg-darkGreen/60 backdrop-blur-md animate-in fade-in duration-500"
            onClick={() => setIsMenuOpen(false)}
          ></div>
          
          {/* Menu Card */}
          <div className="relative w-full max-w-sm bg-cream border-2 border-gold shadow-2xl rounded-[2.5rem] overflow-hidden flex flex-col animate-[menuIn_0.5s_cubic-bezier(0.16,1,0.3,1)] max-h-[90vh]">
            ...
            {/* Nav Links */}
            <div className="flex flex-col items-center gap-2 py-4 relative z-10 overflow-y-auto">
              {navItems.map((item, idx) => (
                <button
                  key={item.id}
                  onClick={() => { setActivePage(item.id); setIsMenuOpen(false); }}
                  className={`text-xl font-serif tracking-widest py-3 px-8 rounded-full transition-all duration-300 transform animate-[fadeInUp_0.4s_ease-out_forwards] opacity-0 active:scale-95`}
              ...
              <button
                ...
                className="flex items-center gap-3 text-sage font-bold uppercase text-xs tracking-widest animate-[fadeInUp_0.4s_0.45s_forwards] opacity-0 py-3 px-6"
              >
                <Globe size={18} /> {lang === 'it' ? 'Switch to English' : 'Passa all\'Italiano'}
              </button>
            </div>

            {/* CTA in Modal */}
            <div className="p-6 pt-2 relative z-10 mt-auto">
              <button
                onClick={() => { setActivePage('contact'); setIsMenuOpen(false); }}
                className="w-full bg-darkGreen text-white py-5 rounded-full font-bold uppercase text-sm tracking-[0.2em] shadow-xl animate-[fadeInUp_0.4s_0.5s_forwards] opacity-0 active:scale-95 transition-transform"
              >
                {t.bookNow}
              </button>
            </div>
